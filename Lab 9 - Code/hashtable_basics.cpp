...
struct HashTable{
    ...
    void intialize_hashtable(){
        for (int i = 0; i < 4; i++){
            password[i].clear(); // Clear method is called to make sure we start off with an empty string array
        }
    }
    ...
    int hashfunc(string user_name){ // Find this code somewhere
        int sum = 0;
        int hash = 0;
	//add your code below

        for(int i=0; i<user_name.length(); i++){ // Loop through the characters of the user name and add their ASCII values to the sum variable
            sum += user_name[i];
        }
	
        hash = sum % MAX_LENGTH; // Use a usual integer hash func to calculate the has code
	...
	//add an if condition to complete the code here
        if(empty){ // Check if the given slot by the hash code is empty, if so insert the password to that slot
